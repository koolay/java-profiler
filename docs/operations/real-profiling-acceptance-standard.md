...
- When validating an existing non-demo workload, set `JAVA_PROFILER_ACCEPTANCE_LOAD_PATHS` to one or more comma-separated HTTP paths that actually exist on that service, so the acceptance script can drive real CPU/allocation/lock load without relying on the JDK demo endpoints.
