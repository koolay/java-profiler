...
The remaining production-shaped proof is to run the same target filter against the real workload `kd-cosmic-xk/mservice` with a short, bounded collection window and durable evidence. This has elevated risk because earlier direct attach attempts caused a JVM crash. The work must preserve the new Coroot-style "write only if changed" async-profiler asset behavior, avoid broadening the product beyond Java-on-Kubernetes profiling, and capture enough before/after state to prove whether the attach path is safe for `mservice`.
...
- Parameterize or harden the real acceptance path so it can target an existing workload in namespace `kd-cosmic-xk` with service/workload filter `mservice`.
...
- Long-running continuous profiling of `mservice`.
- Directly editing or restarting `mservice` unless the existing acceptance workflow already requires temporary profiling metadata and the operator context permits it.

## Requirements Trace

- R1, R2, R3, R5, R6, R27, R28: target `mservice` through explicit Kubernetes metadata/filtering and keep the run bounded.
...
- AE2, AE3, AE5, AE8, AE9, AE10, AE11, AE13, AE15: carry forward the isolated acceptance coverage and repeat the key profile/status UI proof against `mservice` where safe.
...
- Treat `mservice` acceptance as a validation mode for an existing workload, not an install mode for a synthetic BusyApp. The script should be able to skip workload creation and still apply/check profiler deployment filters.
...
- Before attaching, record baseline Pod status and restart counts; after profiling, record the same state and fail or clearly flag any restart increase for `mservice`.
...
- Ensure Helm target filters can be set to `kd-cosmic-xk/mservice` without accidentally profiling unrelated Java Pods.
...
- The generated summary shows namespace `kd-cosmic-xk`, service `mservice`, profiler namespace, release, and artifact directory.
...
- Capture the same data after the profiling window and compare restart counts for `mservice`.
- Preserve the existing non-empty profile/thread/deadlock/ingestion assertions, but add a clear safety failure if `mservice` restarts during the run.
...
- If profile samples, stacks, thread snapshots, and ingestion rows are non-empty, the summary reports the counts for `mservice`.
...
- Document the mservice validation command and the expected evidence artifacts only if the script or behavior changes.
...
5. Run the short-window `kd-cosmic-xk/mservice` acceptance, saving screenshots/video and summary under a timestamped `/tmp/java-profiler-mservice-*` directory.
...
- Short-window real acceptance against `kd-cosmic-xk/mservice` with:
  - non-empty target status for `kd-cosmic-xk/mservice`
  - non-empty CPU, allocation, and lock profile evidence when the workload is active enough
  - collector/backend ingestion rows
  - before/after restart counts showing no new `mservice` restart
...
- The current Kubernetes context is the intended cluster for `kd-cosmic-xk/mservice` validation.
- The operator context allows applying or using temporary profiling metadata/filters for `mservice`.
- `mservice` is HotSpot-compatible and active enough during the short window to produce meaningful CPU/allocation/lock samples.
