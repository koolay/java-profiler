...
test("real cluster Java profiling workbench exposes status, CPU, Wall Clock, I/O, GC, deadlock, and ingestion surfaces", async ({ page }) => {
  ...
  const currentFocusChip = focusPath.getByRole("button").last();
  const initialFocusName = await currentFocusChip.textContent();
  const nextFocusIndex = rowCount > 2 ? 2 : 1;
  await focusedRows.nth(nextFocusIndex).click();
  await expect(focusPath.getByRole("button").last()).not.toHaveText(initialFocusName ?? "");
  await focusState.getByRole("button", { name: "Back" }).click();
  await expect(focusPath.getByRole("button").last()).toHaveText(initialFocusName ?? "");
