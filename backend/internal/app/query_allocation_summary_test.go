...
func TestAllocationSummaryRanksPathsSelfFramesAndInsights(t *testing.T) {
	...
	if !got.Coverage.NewestProfileEnd.Equal(now.Add(time.Second)) {
		t.Fatalf("newest profile end = %s", got.Coverage.NewestProfileEnd)
	}
}
...
func TestAllocationSummaryNormalizesScopeAndRejectsInvalidQueries(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected invalid jvm process id error")
	}
	...
	if err == nil {
		t.Fatal("expected namespace-only range guardrail error")
	}
}
...
func TestAllocationSummaryFiltersContainerAndJVM(t *testing.T) {
	repo := clickhouse.NewProfileRepository()
	now := time.Unix(100, 0).UTC()
	samples := []clickhouse.ProfileSample{
		{Target: domain.TargetIdentity{Namespace: "prod", Service: "checkout", Pod: "checkout-1", Container: "app", ProcessID: 101}, ProfileType: domain.ProfileTypeAllocBytes, StartedAt: now, EndedAt: now.Add(time.Second), StackID: "wanted", Frames: []string{"root", "Wanted.alloc"}, Value: 7},
		{Target: domain.TargetIdentity{Namespace: "prod", Service: "checkout", Pod: "checkout-1", Container: "sidecar", ProcessID: 202}, ProfileType: domain.ProfileTypeAllocBytes, StartedAt: now, EndedAt: now.Add(time.Second), StackID: "ignored", Frames: []string{"root", "Ignored.alloc"}, Value: 99},
	}
	if err := repo.InsertProfileBatch(context.Background(), "batch-filter", samples); err != nil {
		t.Fatalf("insert failed: %v", err)
	}
	...
	if got.Coverage.TotalValue != 7 || len(got.TopPaths) != 1 || got.TopPaths[0].LeafFrame != "Wanted.alloc" {
		t.Fatalf("summary did not apply container/jvm filters: coverage=%+v paths=%+v", got.Coverage, got.TopPaths)
	}
}

func TestAllocationSummaryTruncatesNodeLimit(t *testing.T) {
	repo := clickhouse.NewProfileRepository()
	now := time.Unix(100, 0).UTC()
	samples := []clickhouse.ProfileSample{
		{Target: domain.TargetIdentity{Namespace: "prod", Service: "checkout"}, ProfileType: domain.ProfileTypeAllocBytes, StartedAt: now, EndedAt: now, StackID: "a", Frames: []string{"root", "A.alloc"}, Value: 3},
		{Target: domain.TargetIdentity{Namespace: "prod", Service: "checkout"}, ProfileType: domain.ProfileTypeAllocBytes, StartedAt: now, EndedAt: now, StackID: "b", Frames: []string{"root", "B.alloc"}, Value: 2},
	}
	if err := repo.InsertProfileBatch(context.Background(), "batch-node-limit", samples); err != nil {
		t.Fatalf("insert failed: %v", err)
	}
	got, err := QueryAllocationSummary(context.Background(), repo, nil, nil, AllocationSummaryQuery{
		Namespace:      "prod",
	...
	if len(got.TopPaths) != 1 || !hasReason(got.Coverage.PartialReasons, "node_limit") {
		t.Fatalf("expected node-limited paths, coverage=%+v paths=%+v", got.Coverage, got.TopPaths)
	}
}

func TestAllocationSummaryReportsNoStackFrames(t *testing.T) {
	repo := clickhouse.NewProfileRepository()
	now := time.Unix(100, 0).UTC()
	if err := repo.InsertProfileBatch(context.Background(), "batch-empty-frames", []clickhouse.ProfileSample{
		{Target: domain.TargetIdentity{Namespace: "prod", Service: "checkout"}, ProfileType: domain.ProfileTypeAllocBytes, StartedAt: now, EndedAt: now, StackID: "empty", Value: 5},
	}); err != nil {
		t.Fatalf("insert failed: %v", err)
	}
	got, err := QueryAllocationSummary(context.Background(), repo, nil, nil, AllocationSummaryQuery{
	...
	if got.Coverage.HasData || got.Coverage.EmptyState != "no_stack_frames" {
		t.Fatalf("coverage = %+v", got.Coverage)
	}
}
